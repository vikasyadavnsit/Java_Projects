package com.battleramp.handler;
...
@Component
public class HomeCallbackURLHandler {
	static Logger log = Logger.getLogger(HomeCallbackURLHandler.class);
	...
	public void facebookURLHandler(HttpServletRequest request, Model model) {
		...
		try {
			...
			try {
				...
				builder2.setParameter("redirect_uri", "http://localhost:8080/soap/facebookCallbackURL");
		...
		try {
			...
			if (response2 != null && HttpStatus.SC_OK == response2.getStatusLine().getStatusCode()) {
				...
				if (responseJSON != null) {
					List<NameValuePair> params = URLEncodedUtils.parse(responseJSON, Charset.forName("UTF-8"));
